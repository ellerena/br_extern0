...
static ssize_t cd_read(struct file *filep, char *buffer, size_t len, loff_t *offset)
{
   ...
   switch(mcom)
   {
      case COM_K:   /*1*/
      case COM_CAP: /*2*/
         i[0] = RD_OFF32(INFO_DATE_OFFSET);
         i[1] = RD_OFF32(INFO_TIME_OFFSET);
         i[2] = RD_OFF32(INFO_VERSION_OFFSET);
         i[3] = RD_OFF32(INFO_PRODUCT_OFFSET);
         break;

      case COM_RST: /*0*/
      case COM_DMP: /*3*/
         sprintf((void*)i, "Paris et Libere");
         break;
   }

   copy_to_user(buffer, (void*)&i, count);
